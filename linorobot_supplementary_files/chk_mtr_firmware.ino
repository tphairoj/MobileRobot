    ...
    if ((millis() - prev_control_time) >= (1000 / COMMAND_RATE))
    {
        if((countDelay++)%5 == 0)
        {
            ...
            if( iPWM >= maxPWM )
                ...
                pwmDirection = 1; 
            if( iPWM == 0 && pwmDirection == 1 )
            {
                iPWM_Mot1 = iPWM_Mot2 = 0;
                iStep += 1;
                if( iStep == 3 )
                    iStep = 0;
            }
            ...
            {
                iPWM_Mot1 = iPWM; 
                motor1_controller.spin(iPWM);
            }
            else if( iStep == 1 )
            {
                iPWM_Mot2 = iPWM;
                motor2_controller.spin(iPWM);
            }
            else if( iStep == 2 )
            {
                iPWM_Mot1 = iPWM_Mot2 = iPWM;
                motor1_controller.spin(iPWM);
                motor2_controller.spin(iPWM);
            }
       }
...
void moveBase()
{
    ...
    int current_rpm3 = 0;//motor3_encoder.getRPM();
    int current_rpm4 = 0;//motor4_encoder.getRPM();
...
void printDebug()
{
    ...
    sprintf (buffer, "Encoder FrontLeft  : %ld %d", motor1_encoder.read(), iPWM_Mot1);
    nh.loginfo(buffer);
    sprintf (buffer, "Encoder FrontRight : %ld %d", motor2_encoder.read(), iPWM_Mot2);
    nh.loginfo(buffer);
